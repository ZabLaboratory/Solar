...
describe("orionBundleUrl()", () => {
  it("maps the local live WS URL to the local render-bundle URL", () => {
    const resolve = orionBundleUrl(
      "ws://127.0.0.1:4007/orion/api/v1/show/stream.lsdp?token=x",
    );
    expect(resolve("SCENE", "sha256:V")).toBe(
      "http://127.0.0.1:4007/orion/api/v1/scenes/SCENE/render-bundle?v=sha256%3AV",
    );
  });
  ...
  it("percent-encodes scene id and version for the local runtime", () => {
    const resolve = orionBundleUrl(
    ...
    expect(resolve("s pace", "sha256:a/b+c")).toBe(
      "http://127.0.0.1:4007/orion/api/v1/scenes/s%20pace/render-bundle?v=sha256%3Aa%2Fb%2Bc",
    );
  });

  it("falls back to stripping a /show/* suffix when the exact WS suffix drifts", () => {
    const resolve = orionBundleUrl(
      "ws://127.0.0.1:4007/orion/api/v1/show/stream",
    );
    expect(resolve("SCENE", "sha256:V")).toBe(
      "http://127.0.0.1:4007/orion/api/v1/scenes/SCENE/render-bundle?v=sha256%3AV",
    );
  ...
  it("normalises a trailing slash on the API root", () => {
    ...
    expect(resolve("SCENE", "v1")).toBe(
      "http://127.0.0.1:4007/orion/api/v1/scenes/SCENE/render-bundle?v=v1",
    );
  });
